...
module RS::CSP::Evaluator
  ...
  class UCBSatisfaction < Satisfaction
    ...
    #
    # @param [RS::CSP::Evaluator::UCBSatisfaction] ohter 比較対象となるRS::CSP::Evaluator::UCBSatisfactionオブジェクト
  ...
  class LPBSatisfaction < Satisfaction
    ...
    #
    # @param [RS::CSP::Evaluator::LPBSatisfaction] ohter 比較対象となるRS::CSP::Evaluator::LPBSatisfactionオブジェクト
    # @return [true, false] このオブジェクトの方が充足度が高ければtrue, そうでなければfalse。
    def > (other)
      0.upto(@error.length-1).each {|i|
